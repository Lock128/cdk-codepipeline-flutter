    ...
    public CdkPipelineStack(final Construct parent, final String id, final String branch, final StackProps props) {
        ...
        final CodePipeline pipeline = CodePipeline.Builder.create(this, getCodepipelineName(branch))
                ...
                .synth(CodeBuildStep.Builder.create("SynthStep")
                        .input(CodePipelineSource.connection("lock128/cdk-codepipeline-flutter", branch, ConnectionSourceOptions.builder().connectionArn(connectionArn).build()))
        ...
        pipeline.addStage(new FlutterBuildStage(this, "FlutterBuildStage"), AddStageOpts.builder().pre(List.of(
				ShellStep.Builder.create("Install Flutter")
				...
				.build()))
        		.post(List.of(
        				ShellStep.Builder.create("Execute Flutter Build and CodeCov")
        				.commands(List.of("cd ui", "flutter test", "flutter build web","bash ../start_codecov.sh"))
        				.build(), ShellStep.Builder.create("Upload Flutter")
        				.commands(List.of("aws s3 sync ui/build/web s3://cdk-codepipeline-flutter"))
        				.build()))
        		.build());
